...
# TODO: Uncomment this to go back to 10 fold cv
...
#     train_WA_runs = np.zeros(fold*2)
#     val_UA_runs = np.zeros(fold*2)
#     val_WA_runs = np.zeros(fold*2)
#     test_UA_runs = np.zeros(fold*2)
#     test_WA_runs = np.zeros(fold*2)
#
...
# TODO: Delete this after running corrected 5 fold cv
def KfoldCv(fold, seed, config, label_files, sessions, max_text_tokens, max_audio_tokens, device, data_dir, models_dir):
    ...
    kfold_total = [tuple((np.array(([0,1,3,4])), np.array(([2])), ['_F', '_M'])), tuple((np.array(([0,1,2,4])), np.array(([3])), ['_F', '_M'])),tuple((np.array(([1,2,3,4])), np.array(([0])), ['_F', '_M'])),tuple((np.array(([0,2,3,4])), np.array(([1])), ['_F', '_M'])), tuple((np.array(([0,1,2,3])), np.array(([4])), ['_F', '_M']))]
    ...
    # Loop over session folds
    for run, split_sess in tqdm(enumerate(kfold_total), total=fold):
