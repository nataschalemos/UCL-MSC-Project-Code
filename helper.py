"""
Script adapted from: https://github.com/shamanez/MAIA-Data-Processing/tree/master/Preprocessing_scripts/iemocap/data_extract
changes: set pre-processing of text also set all to lowercase.
...
EMOTION_ENCODING = {'ang': 0, 'hap': 1, 'exc': 2, 'sad': 3, 'neu': 4, 'fru': 5, 'sur': 6, 'xxx': 7}
...
#         end = e['end']
#
#         with open('/Volumes/TOSHIBA EXT/Code/Paper 2/labels_train_v.txt', 'a') as label_file:
#             text = filename + '\t' + str(EMOTION_ENCODING[label]) + '\n'
#             label_file.write(text)
#
#         targetname = '/Volumes/TOSHIBA EXT/Code/Paper 2/Video/' + filename
#
...
def split_wav(wav, emotions, ex, sess):
    ...
    for ie, e in enumerate(emotions):
        ...
        with open('/Volumes/TOSHIBA EXT/Code/IEMOCAP/labels_train_a.txt', 'a') as label_file:
            text = filename + '\t' + str(EMOTION_ENCODING[label]) + '\n'
            label_file.write(text)

        audio_chunk = array.array(array_type, e['left'].tobytes())
        ofile = wave.open('/Volumes/TOSHIBA EXT/Code/IEMOCAP/Audio/' + filename, 'w')
...
def split_text(txt, emotions, ex, sess):
    #emotions_used = np.array(['ang', 'exc', 'fru', 'sad', 'neu', 'sur', 'hap'])
    emotions_used = np.array(['ang', 'hap', 'exc', 'sad', 'neu'])
    ...
    for ie, e in enumerate(emotions):
        ...
        with open('/Volumes/TOSHIBA EXT/Code/IEMOCAP/labels_train_t.txt', 'a') as label_file:
            text = filename + '\t' + str(EMOTION_ENCODING[label]) + '\n'
            label_file.write(text)

        with open('/Volumes/TOSHIBA EXT/Code/IEMOCAP/Text/' + filename, 'a') as source_file:
            # text =  filename + '\t' + str(EMOTION_ENCODING[label]) + '\n'
            source_file.write(real_text.lower())
