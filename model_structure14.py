...
# Class for training only with LHS
class LHS(nn.Module):
...
# Class for training RHS
class RHS_train(nn.Module):  # (note: this class defines the whole model)
    def __init__(self, roberta, speechBert, fuse_dim=128, dropout_rate=0.0, output_dim=4, freeze_models=False):  # NOTE: changed output_dim from 5 to 4
        super(RHS_train, self).__init__()
        ...
        self.b2 = speechBert
        self.b3 = Time_async()
        self.layer_cat = nn.Linear(64 * 5, fuse_dim)
        self.layer_out = nn.Linear(fuse_dim, output_dim, bias=False)
        #self.dropout = nn.Dropout(p=dropout_rate)
    ...
    def forward(self, x, y, z):
        ...
        out = F.relu(self.layer_cat(z))  # 200,128 # TODO: check if dim fuse_dim=128 is appropriate
        out = self.layer_out(out)  # 200,4
