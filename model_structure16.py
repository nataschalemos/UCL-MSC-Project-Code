...
class SpeechBert_RHS(nn.Module):  # (note: this class defines the whole model)
    def __init__(self, roberta, speechBert, fuse_dim=128, dropout_rate=0.0, output_dim=4, freeze_models=False):  # NOTE: changed output_dim from 5 to 4
        super(SpeechBert_RHS, self).__init__()
        ...
        self.b2 = speechBert
        self.b3 = Time_async()
        self.layer_cat = nn.Linear(768 + 64 * 5, fuse_dim)
        self.layer_out = nn.Linear(fuse_dim, output_dim, bias=False)
        #self.dropout = nn.Dropout(p=dropout_rate)
    ...
    def forward(self, x, y, z):
        y = self.b2.extract_features(y)[:, 0, :]
        z, A3 = self.b3(z)  # z: 200,320
        out = torch.cat((y, z), 1)  # 200,2112
